...
import { API_URL, createTask, getDemoStatus, getEvaluation, getProjects, getTask, GuardedTask, mutateTask, scanInventory } from "@/lib/api";

type View = "overview" | "inventory" | "task" | "execution" | "report" | "bench";
const nav: { id: View; label: string; href: string; icon: typeof Shield }[] = [
  ...
  { id: "report", label: "Report", href: "/report", icon: FileCode2 },
...
export function ControlPlane({ initialView = "overview" }: { initialView?: View }) {
  ...
  const [taskInstruction, setTaskInstruction] = useState(instruction);
  ...
  useEffect(() => {
    ...
    getDemoStatus().then(setStatus).catch(() => setStatus({}));
  ...
  const start = () => run(async () => mutateTask((await createTask(taskInstruction, failure)).id, "plan"));
  const scan = async () => {
    setBusy(true); setError("");
    try { setProjects(await scanInventory()); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Inventory scan failed"); }
    finally { setBusy(false); }
  };
  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <div className="side-status"><span className="pulse"/><div><b>{String(status.mode ?? "Demo")} environment</b><small>Isolated sandbox required</small></div></div>
      <div className="side-foot"><LockKeyhole size={14}/> Local demo auth</div>
    </aside>
    <div className="workspace">
      <header><div><span className="eyebrow">DEVELOPER CONTROL PLANE</span><h1>{nav.find((item) => item.id === view)?.label}</h1></div><div className="provider"><span>EXECUTION PROVIDER</span><b><i/> {String(status.provider ?? task?.provider ?? "loading")}</b></div></header>
      <main className="content">
        ...
        {view === "inventory" && <Inventory projects={projects} scan={scan} busy={busy}/>}
        {view === "task" && <TaskBuilder task={task} instruction={taskInstruction} setInstruction={setTaskInstruction} planner={String(status.planner_provider ?? "loading")} failure={failure} setFailure={setFailure} start={start} approve={approveBoundary} busy={busy}/>}
        {view === "execution" && <Execution task={task} blocked={blocked} start={start} execute={execute} approve={approveAction} busy={busy}/>}
        {view === "report" && <ExecutionReport task={task}/>}
...
function Overview({ task, evaluation, start, busy }: { task: GuardedTask | null; evaluation: Record<string, unknown>; start: () => void; busy: boolean }) {
  ...
  return <><section className="hero"><div><span className="kicker"><Fingerprint size={14}/> INTENT-BOUND EXECUTION</span><h2>Fast agents.<br/><em>Hard boundaries.</em></h2><p>Translate developer intent into enforceable resource limits, intercept scope drift before execution, and prove what stayed untouched.</p><button className="primary" onClick={start} disabled={busy}><Play size={16}/>{busy ? "Preparing…" : "Run signature demo"}<ArrowRight size={15}/></button></div><div className="orbit"><Shield size={68}/><span className="o1">FILES</span><span className="o2">SERVICES</span><span className="o3">DATABASES</span><span className="o4">NETWORK</span></div></section>
    <section className="metric-grid"><Metric label="GUARDED EXECUTIONS" value={task ? "01" : "00"} note="current session"/><Metric label="BLOCKED ACTIONS" value={task?.actions.length ? String(task.actions.filter((row)=>String((row.decision as Record<string,unknown>).decision).startsWith("BLOCK")).length).padStart(2,"0") : "00"} note="before execution" tone="amber"/><Metric label="PROTECTED INTEGRITY" value={metrics ? `${Math.round((metrics.protected_resource_integrity_rate ?? 0)*100)}%` : "—"} note={metrics ? "EngageFlow unchanged" : "not measured"}/><Metric label="ROLLBACK SUCCESS" value={task?.rolled_back ? "100%" : "—"} note="task-scoped restore"/></section>
...
function Inventory({ projects, scan, busy }: { projects: unknown[]; scan: () => void; busy: boolean }) {
...
function TaskBuilder({ task, instruction, setInstruction, planner, failure, setFailure, start, approve, busy }: { task: GuardedTask|null; instruction:string; setInstruction:(value:string)=>void; planner:string; failure:boolean; setFailure:(v:boolean)=>void; start:()=>void; approve:()=>void; busy:boolean }) {
  const plan = task?.plan as Record<string, unknown> | null;
  return <><div className="page-intro"><div><span className="eyebrow">NEW GUARDED TASK</span><h2>Declare the outcome. Approve the boundary.</h2></div></div><div className="task-layout"><div className="panel composer"><label htmlFor="task-instruction">Task instruction</label><textarea id="task-instruction" value={instruction} onChange={(event)=>setInstruction(event.target.value)}/><label className="toggle"><input type="checkbox" checked={failure} onChange={(event)=>setFailure(event.target.checked)}/><span/> Inject failed RD Social health check to prove rollback</label><button className="primary" onClick={start} disabled={busy || instruction.trim().length < 8}><Fingerprint size={16}/>{task ? "Regenerate boundary" : "Interpret task"}</button><div className="model-note"><Spark/><span><b>Planner contribution · {planner}</b>Task interpretation, boundary proposal, risk explanation, validation and rollback plans.</span></div></div><div className="panel boundary"><div className="boundary-head"><PanelTitle icon={Shield} title="Boundary manifest"/><span className="provider-chip">{planner}</span></div>{plan ? <><div className="intent"><span>INTERPRETED INTENT</span><p>{String(plan.interpreted_intent)}</p></div><div className="boundary-cols"><BoundaryList title="ALLOWED" tone="allow" values={["/workspace/projects/rdsocial","rdsocial-api","database:rdsocial",":8101"]}/><BoundaryList title="PROTECTED" tone="protect" values={["/workspace/projects/engageflow","engageflow-api","database:engageflow",":8201"]}/></div><div className="risk"><AlertTriangle size={16}/><span><b>Risk assessment</b>{String(plan.risk_summary)}</span></div><button className="approve" onClick={approve} disabled={task?.status === "boundary_approved"}><Check size={16}/>{task?.status === "boundary_approved" ? "Boundary approved" : "Approve manifest"}</button></> : <div className="empty"><Shield size={40}/><b>No boundary proposed</b><span>Interpret the task to generate a deterministic manifest.</span></div>}</div></div></>;
}
...
function ExecutionReport({task}:{task:GuardedTask|null}) {
  if (!task?.report) return <div className="empty-page"><FileCode2 size={48}/><h2>No completed report</h2><p>Complete the guarded execution to generate evidence from its audit events.</p><Link className="primary" href="/execution">Open execution</Link></div>;
  return <><div className="page-intro"><div><span className="eyebrow">EXECUTION EVIDENCE</span><h2>Guarded execution report</h2><p>Generated from the task audit chain after validation and optional target-only rollback.</p></div></div><Report task={task}/></>;
}
